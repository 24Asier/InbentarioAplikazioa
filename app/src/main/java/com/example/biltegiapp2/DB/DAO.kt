...
interface DAO {
    ...
    fun getAllAlbaran(): List<Albaran>

    @Query("SELECT * FROM Interakzioa")
    fun getAllInterakzioak(): List<Interakzioa>
    ...
    @Insert
    fun insertProduktua(produktua: Produktua):Long
    ...
    @Delete
    fun deleteProfila(profila: Profila)

    @Delete
    fun deleteProduktua(produktua: Produktua)

    @Delete
    fun deleteAlbaran(albaran: Albaran)

    @Query("SELECT p.* FROM Produktua p JOIN Interakzioa i ON p.prodId = i.prodId JOIN Profila pr ON i.profilId = pr.profilID WHERE pr.email = :email")
    fun getProductsForUser(email: String): List<Produktua>

    @Query("SELECT * FROM Interakzioa WHERE prodId = :productId")
    fun getInteractionsForProduct(productId: Int): List<Interakzioa>
}